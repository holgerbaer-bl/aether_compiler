...
- **Execution Backend**: The engine is actively migrating intensive workloads away from recursive AST evaluation (`ExecutionEngine`) toward flat instruction processing (`Compiler` emitting `OpCode` to the new `VM`). Expect new structures in `src/vm`.
