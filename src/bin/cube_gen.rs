// Generates cube_demo.aec
// Features: Time(), Sin(), Cos(), Mat4Mul(), and RenderMesh with Uniform Matrix passing.
...
fn main() {
    ...
    stmts.push(Node::InitWindow(
        Box::new(int(800)),
        Box::new(int(600)),
        Box::new(str_lit("AetherCore 3D Cube (AST MVP Matrices)")),
    ));

    stmts.push(Node::InitGraphics);

    // Bind Group 0 = Uniform MVP Matrix, 16 floats.
    // We hardcode 36 vertex points for a cube in the shader
    let wgsl = r#"
        struct Uniforms {
            mvp: mat4x4<f32>,
        }
    ...
    // Simple perspective projection FOV 45, Aspect 800/600, Near 0.1, Far 100.0
    // wgpu uses Z NDC [0, 1]
    ...
    // t = Time() * speed
    loop_body.push(assign("t", mul(Node::Time, float(1.5))));

    // Model Rotation Matrix (Y & X combined or just Y)
    // Let's do simple Y rotation: [cos(t), 0, -sin(t), 0, 0, 1, 0, 0, sin(t), 0, cos(t), 0, 0, 0, 0, 1]
    let s = sin(var("t"));
    let c = cos(var("t"));
    ...
    let dest_path = "target/cube_demo.aec";
